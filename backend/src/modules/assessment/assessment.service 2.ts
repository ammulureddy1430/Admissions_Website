...
export class AssessmentService {
  ...
  async getSubmissions(schoolId: string) {
    ...
    // Games remain separate result records for scoring and auditability, but the
    ...
    for (const submission of libraryGameSubmissions) {
      const studentId = submission.gameResult.studentId;
      gameSubmissionsByStudent.set(studentId, [...(gameSubmissionsByStudent.get(studentId) || []), submission]);
    }
    const consolidatedGameSubmissions = [...gameSubmissionsByStudent.values()].map((rows) => {
      ...
      return {
        ...
        tabSwitchCount: rows.reduce((sum, row) => sum + Number(row.tabSwitchCount || 0), 0),
        fullscreenExitCount: rows.reduce((sum, row) => sum + Number(row.fullscreenExitCount || 0), 0),
        assessment: { ...representative.assessment, title: 'Complete Game Assessment', subject: 'All Assigned Games' },
        gameResult: {
          ...representative.gameResult,
    ...
    return [...submissions.map((submission) => ({ ...submission, submissionType: 'STANDARD' })), ...gameBasedAssessmentSubmissions, ...consolidatedGameSubmissions]
