...
export class GamesService implements OnModuleInit {
  ...
  private analyticsFor(result: any) {
    return (
      ...
      (['CATCH_THE_TARGET', 'MENTAL_ROTATION', 'WATER_JUGS', 'TANGRAM_BUILDER', 'SOKOBAN', 'RACING_STRATEGIST', 'PLAYMAKER', 'CLIMBING_CHALLENGE', 'DETECTIVE_INVESTIGATION', 'PRECISION_ARCHERY', 'WAVE_RIDER', 'STEALTH_ESCAPE', 'DRIFT_RACER', 'QUICK_SWITCH'].includes(
  ...
  async list(schoolId: string, query: Record<string, string>) {
    const games = await this.prisma.game.findMany({
      where: {
        ...
        slug: { in: GAME_CATALOG.map((game) => game.slug) },
        ...(query.status && { status: query.status }),
