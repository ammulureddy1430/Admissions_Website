...
export default function MagicPaintGame({ disabled=false, durationSeconds=MAGIC_PAINT_DURATION_SECONDS, onComplete }:{disabled?:boolean;sound?:boolean;durationSeconds?:number;onComplete:(metrics:MagicPaintScores)=>void|Promise<void>}) {
  ...
  const metrics=useRef(engine.emptyMetrics()); const objectStarted=useRef(0); const interactions=useRef(0); const finished=useRef(false); const completeRef=useRef(onComplete);
  useEffect(()=>{objectStarted.current=performance.now()},[]); useEffect(()=>{completeRef.current=onComplete},[onComplete]);
